...
public class BcRecordServiceImpl extends BaseServiceImpl<BcRecord,Long> implements BcRecordService{
    ...
    @Override
    public ActionResult bcRecordMealSave(String appId,Long id,int status,Integer orderMeal, Integer bcType, Integer bcRs, Long deptId) throws Exception{
        BcConfig config =bcConfigService.getConfigByAppId(appId, deptId);
        Boolean userNeedApprove = config.isUserNeedApprove();
        Boolean lunchCanMeal = config.isLunchCanMeal(); // 中餐
        Boolean dinnerCanMeal = config.isDinnerCanMeal(); // 晚餐
        //不需要审核
        if(!userNeedApprove){
//            //中午报餐状态开启
//            if(lunchCanMeal){
//                return this.save(appId,id,orderMeal, bcType);
//            }else{
//                return ActionResult.error(2,"中午报餐功能未开启");
//            }
            return this.save(appId,id,orderMeal, bcType, bcRs);
        }else{
//            if(lunchCanMeal){
//                if(status == 1){
//                    return this.save(appId,id,orderMeal, bcType);
...
//            }
     ...
     * @throws Exception
     */
    public ActionResult save(String appId,Long id,Integer orderMeal, Integer bcType, Integer bcRs)throws Exception{
        BcRecord bcRecord = new BcRecord();
        bcRecord.setAddTime(new Date());
        bcRecord.setAppId(appId);
        bcRecord.setUserId(id);
        bcRecord.setBcType(bcType);
        bcRecord.setBcChannel(BcRecordCons.BC_CHANNEL_MANUAL);
        bcRecord.setBcRs(bcRs);
        if(1 == orderMeal){
    ...
    public ActionResult getByUserIdAndDinTime(String appId,Long id,String date) throws Exception {
        List<BcRecord> bcRecords = bcRecordRepository.getByUserIdAndDinTime(appId,id,date);
        if (bcRecords != null){
            return  ActionResult.ok(bcRecords);
        }else{
    ...
    @Override
    public List<Map<String, Object>> getBcRecordListByDinTime(String appId, String date,Long deptId, String openId,int currentPage,int pageSize) throws Exception {
        ...
        String sql = "select DATE_FORMAT (r.addTime, '%m-%d %H:%i:%s') as addTime ," +
                ...
                "  case  when  r.bc_type ='0' then '早餐'" +
                "  when r.bc_type ='1' then '中餐'" +
                "  when r.bc_type ='2' then '晚餐' end as bcType, "+
                "  r.bc_rs as bcRs, " +
                ...
                " where r.deletestatus = 0 and r.app_id =:appId and r.dintime like concat('%',:dintime, '%') ";
        if(CommUtils.isNotNull(deptId)){
            sql+="and d.id=:deptId";
            map.put("deptId",deptId);
        }
        if (CommUtils.isNotNull(openId)) {
            sql += " and w.openid =:openId";
            map.put("openId",openId);
        }

        if (!CommUtils.isNotNull(deptId) && !CommUtils.isNotNull(openId)) {
            // TODO 取巧 部门id和openId都为空，说明是游客，不给查询报餐记录
