...
public interface BcRecordRepository extends JpaRepository<BcRecord,Long>{
     ...
     * @return
     */
    @Query("select obj from BcRecord obj where obj.appId=:appId and obj.deleteStatus = 0 and obj.userId=:userId and obj.dinTime like concat('%',:dinTime,'%') ")
    List<BcRecord> getByUserIdAndDinTime(@Param("appId") String appId,@Param("userId")Long userId, @Param("dinTime")String dinTime);
     ...
     * @return
     */
    @Query("select NULLIF(sum(NULLIF(r.bcRs, 0)), 0) from BcRecord  r join BcUser u on (r.appId=u.appId and r.userId =u.id) where r.appId=:appId and  r.dinTime like concat('%',:dinTime,'%')")
     ...
     */
    @Modifying
    @Query("update BcRecord obj set obj.deleteStatus = 1 where obj.appId=:appId and obj.userId=:userId and obj.id =:id")
