...
public interface BcUserDepartmentRepository extends JpaRepository<BcUserDepartment,Long> {
     ...
     * @return
     */
    @Query("select NULLIF(sum(NULLIF(r.bcRs, 0)), 0) as count,d.name,d.id from BcUserDepartment d left join BcUser u on (d.id=u.userDepartmentId) left  join BcRecord r on(u.id =r.userId AND u.appId= r.appId and r.dinTime like concat(:dinTime,'%') ) where  d.appId=:appId   group by d.name")
     ...
     * @return
     */
    @Query("select obj from BcUserDepartment obj where obj.appId = :appId and obj.inuse = 1")
    List<BcUserDepartment> getDeptByAppId(@Param("appId")String  appId);
     ...
     * @return
     */
    @Query("select obj from BcUserDepartment obj where obj.appId = :appId and obj.id=:id and obj.inuse = 1")
