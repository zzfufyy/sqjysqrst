...
public interface BcUserRepository extends JpaRepository<BcUser,Long> {
    ...
    @Modifying
    @Query("update  BcUser set userDwId='1'  where appId =:appid and id=:id")
    int updatedwIdById(@Param("appid")String appid,@Param("id")Long id);

    @Modifying
    @Query("update  BcUser set userDwId=null  where appId =:appid and id=:myid")
    int updatemydwIdById(@Param("appid")String appid,@Param("myid")Long myid);
