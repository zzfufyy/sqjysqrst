...
public class BcUserDepartmentController {
     ...
     * @return
     */
    @ApiOperation(value = "查询用户部门信息", tags = {"bcDepartment"}, notes = "")
    @GetMapping("/getBcUserDepartmentById")
    public ActionResult getBcUserDepartmentList(@PathVariable String appid, Long id) {
        try {
            BcUserDepartment department= this.bcUserDepartmentService.getBcUserDepartment(appid, id);
            return ActionResult.ok(department);
        } catch (Exception e) {
    ...
    @RequestMapping("/save")
    public ActionResult save(@PathVariable String appid,String name, String canteenPlace,String subid,String openid){
        try{
            BcUserDepartment department= new BcUserDepartment();
            department.setAppId(appid);
            department.setName(name);
            department.setCanteenPlace(canteenPlace);
            department.setSubid(subid);
            department.setSubopenid(openid);
            department.setInuse("0");
