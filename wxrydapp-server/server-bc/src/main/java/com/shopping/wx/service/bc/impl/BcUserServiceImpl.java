...
public class BcUserServiceImpl  extends BaseServiceImpl<BcUser,Long> implements  BcUserService {
    ...
    @Override
    public ActionResult bcUserRegister(String appId,BcUserAddForm bcUserAddForm,String sfsh)throws Exception {
        Map map = new HashMap(2);
        BcUser bcUser = new BcUser();
        BcConfig config =bcConfigService.getConfigByAppId(appId, Long.parseLong(bcUserAddForm.getUserDepartmentId()));
    ...
    public ActionResult bcUserRegister(String appId,BcUserAddForm bcUserAddForm)throws Exception {
        ...
        //如果用户不需要审核就给予状态为1：已激活
//        if (!userNeedApprove ) {
//            bcUser.setStatus(bcUserAddForm.getStatus());
//        } else {
//
//            bcUser.setStatus(BcUserCons.STATUS_ACTIVE);
//        }
    ...
    @Override
    public List<Map<String, Object>> getUserPageList(BcUserQueryForm queryForm,int currentPage,int pageSize) throws Exception {
        Map<String,Object> map = new HashMap<>(1);
        map.put("appId",queryForm.getAppId());
        String sql ="select  u.id,u.name,DATE_FORMAT (u.addTime, '%Y-%m-%d %H:%i:%s') as addtime,u.mobile,d.name as department,u.status,u.user_department_id from bc_user u"+
    ...
    @Override
    public int updatedwIdById(String appid,Long id) throws Exception {
        return this.bcUserRepository.updatedwIdById(appid,id);
    }

    @Override
    public int updatemydwIdById(String appid,Long myid) throws Exception {
        return this.bcUserRepository.updatemydwIdById(appid,myid);
    }
    ...
    public List<Map<String, Object>> getUserList(BcUserQueryForm queryForm) throws Exception {
        ...
        int currentPage=0;
        int pageSize=0;
        return this.getUserPageList(queryForm,currentPage,pageSize);
    }
